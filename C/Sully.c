...
static int  check_executable_name(char **environ)
{
    size_t i;
    char *result;
    ...
    while (environ[i])
    {
        if (!strncmp(environ[i], "_=", 2))
            break ;
        i++;
    }
    if (!environ[i])
        return (0);
    result = strrchr(environ[i], '/');
    if (!result)
        return (0);
    if (strcmp(result + 1, "Sully"))
        return (1);
    return (0);
}
